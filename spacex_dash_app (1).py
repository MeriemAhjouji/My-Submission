...
def get_pie_chart(entered_site,entered_slider):
    ...
    if entered_site == 'ALL':
        ...
        launch_site = spacex_df[spacex_df['Launch Site'] == entered_site]
        print(launch_site)
        class_counts = launch_site['class'].value_counts().reset_index()
        print(class_counts)
        fig = px.pie(class_counts,
        values='count', 
        names='class', 
        title='total successful launches By site: {}'.format(entered_site))
        fig1 = px.scatter(launch_site, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return [fig,fig1]
# TASK 4:
